...
export async function getRelayLogs(
  ...
  const res = await apiFetch<APIResponse<ExecutionLog[]>>(
    `/relays/${id}/logs?limit=${limit}`,
  )
  return res.data ?? []
}
...
export async function getAvailableProviders(): Promise<{
  ...
  const res = await apiFetch<APIResponse<{ providers: string[] }>>('/connections/providers')
